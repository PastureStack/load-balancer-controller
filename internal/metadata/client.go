...
func (client *httpClient) GetName() (string, error) {
	var result string
	return result, client.decode("/name", &result)
}

func (client *httpClient) GetRegionName() (string, error) {
	var result string
	return result, client.decode("/region_name", &result)
}
...
func (client *httpClient) GetServiceInLocalEnvironment(stackName, serviceName string) (Service, error) {
	var result Service
	path := "/stacks/" + url.PathEscape(stackName) + "/services/" + url.PathEscape(serviceName)
	return result, client.decode(path, &result)
}

func (client *httpClient) GetServiceFromRegionEnvironment(regionName, environmentName, stackName, serviceName string) (Service, error) {
	...
	for _, environment := range environments {
		...
		for _, service := range environment.Services {
			if service.StackName == stackName && service.Name == serviceName {
				return service, nil
			}
		}
	}
	return Service{}, errors.New("metadata service was not found")
}

func (client *httpClient) GetServiceInLocalRegion(environmentName, stackName, serviceName string) (Service, error) {
	regionName, err := client.GetRegionName()
	if err != nil {
		return Service{}, err
	}
	return client.GetServiceFromRegionEnvironment(regionName, environmentName, stackName, serviceName)
}
...
func (client *httpClient) GetEnvironments() ([]Environment, error) {
	var result []Environment
	return result, client.decode("/environments", &result)
}

func (client *httpClient) GetServicesFromRegionEnvironment(regionName, environmentName string) ([]Service, error) {
	environments, err := client.GetEnvironments()
	if err != nil {
		return nil, err
	}
	for _, environment := range environments {
		if strings.EqualFold(regionName, environment.RegionName) && strings.EqualFold(environmentName, environment.Name) {
			return append([]Service(nil), environment.Services...), nil
		}
	}
	return []Service{}, nil
}

func (client *httpClient) GetServicesInLocalRegion(environmentName string) ([]Service, error) {
	regionName, err := client.GetRegionName()
	if err != nil {
		return nil, err
	}
	return client.GetServicesFromRegionEnvironment(regionName, environmentName)
}

func (client *httpClient) GetServicesByEnvironmentUUID(environmentUUID string) ([]Service, error) {
	environments, err := client.GetEnvironments()
	if err != nil {
		return nil, err
	}
...
func (client *httpClient) GetStackByName(name string) (Stack, error) {
	var result Stack
	return result, client.decode("/stacks/"+url.PathEscape(name), &result)
}
