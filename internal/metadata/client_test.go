...
func TestClientReadsRegionServiceWithoutUnescapedPathSegments(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		switch request.URL.Path {
		case "/2016-07-29/region_name":
			fmt.Fprint(response, `"local"`)
		case "/2016-07-29/environments":
			fmt.Fprint(response, `[{"region_name":"local","name":"production","services":[{"stack_name":"web","name":"api","uuid":"service-1"}]}]`)
		default:
			http.NotFound(response, request)
		}
	}))
	...
	service, err := NewClient(server.URL+"/2016-07-29").GetServiceInLocalRegion("production", "web", "api")
	if err != nil {
		t.Fatalf("GetServiceInLocalRegion returned an error: %v", err)
	}
	if service.UUID != "service-1" {
		t.Fatalf("service UUID = %q", service.UUID)
	}
}
