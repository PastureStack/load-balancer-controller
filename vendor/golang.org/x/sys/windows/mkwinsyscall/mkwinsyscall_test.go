...
func TestQuotedLogError(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{name: "legitimate", input: "file not found", want: `"file not found"`},
		{name: "newline injection", input: "file not found\nforged log entry", want: `"file not found\nforged log entry"`},
		{name: "carriage return injection", input: "file not found\rforged status", want: `"file not found\rforged status"`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := quotedLogError(errors.New(tt.input))
			if got != tt.want {
				t.Fatalf("quotedLogError() = %q, want %q", got, tt.want)
			}
			if strings.ContainsAny(got, "\r\n") {
				t.Fatalf("quoted error contains a raw line break: %q", got)
			}
		})
	}
}
